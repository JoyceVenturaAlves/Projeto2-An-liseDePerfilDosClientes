...
--(Query 2) Status professional dos leads
-- colunas: status professional, leads(%)
...
-- colunas: faixa salaria, leads(%), ordem
	...
	faixa_de_idade_dos_veiculos as (
		...
		select
			...
			case
				...
				when (extract('year' from visit_page_date) - pro.model_year::int)<=4 then 'até 2 à 4 anos'
				when (extract('year' from visit_page_date) - pro.model_year::int)<=6 then 'até 4 à 6 anos'
				when (extract('year' from visit_page_date) - pro.model_year::int)<=8 then 'até 6 à 8 anos'
				when (extract('year' from visit_page_date) - pro.model_year::int)<=10 then 'até 8 à 10 anos'
